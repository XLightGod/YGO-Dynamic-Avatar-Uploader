...
class AvatarReplacer:
    ...
    def request(self, flow: http.HTTPFlow):
        if "doent.com/upload" not in flow.request.pretty_url:
            return
    ...
    def response(self, flow: http.HTTPFlow):
        if "doent.com/upload" in flow.request.pretty_url:
            if flow.response.status_code == 200:
                ...
                print("替换失败，请重试。")
        elif "/v1/notice/center/unread" in flow.request.pretty_url\
            or "/v1/news" in flow.request.pretty_url:
